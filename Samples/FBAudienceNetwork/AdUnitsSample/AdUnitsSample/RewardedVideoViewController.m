...
@property (nonatomic, strong) IBOutlet UISwitch *skipableRewardedVideoSwitch;
...
- (IBAction)loadAd
{
    ...
    // Configure ad experience
    FBAdExperienceConfig *adExperienceConfig = [self adExperienceConfig];
    self.rewardedVideoAd.adExperienceConfig = adExperienceConfig;
...
{
    NSLog(@"Rewarded video failed with error: %@", error.description);
    self.adStatusLabel.text = [NSString stringWithFormat:@"Rewarded Video ad failed with error: %@", error.localizedDescription];
}
...
{
    FBAdExperienceType adExperienceType = FBAdExperienceTypeRewarded;
    if (self.skipableRewardedVideoSwitch.on) {
        adExperienceType = FBAdExperienceTypeRewardedInterstitial;
    }
    return [[FBAdExperienceConfig alloc] initWithAdExperienceType:adExperienceType];
}
